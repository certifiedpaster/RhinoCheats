...
{
	vec3_t trDelta;
	VectorSubtract(cg_entities[i].nextState.Lerp.pos.trBase, cg_entities[i].currentState.pos.trBase, trDelta);
...
{	
	//When the enemy doesn't have any secondary weapon. (They have AA enabled)
	if (Settings[anti_antiaim].Value.bValue && !cg_entities[i].currentState.secondWeaponID)
	{
		return MyRiotShieldMethod(i, autowall, damage, isVehicle);
	}

	//non riot.
	if (cg_entities[i].nextState.weaponID != 4 && cg_entities[i].nextState.weaponID != 3)
	{
...
int  Aim_t::GetClosestClientByDistance(bool isVehicle)
{
	...
	if (!vTargetInfo.empty())
	{
		...
		DistanceToTarget = vTargetInfo.front().distance;
...
void Aim_t::AutoKnife()
{
	...
	{
		key_input->realmelee.wasPressed = 0;
		bMelee = false;
	}
...
void Aim_t::PerformAimbot() 
{
	...
	{	
		...
		if (Settings[names_stealer].Value.bValue &&
			strcmp(
				Engine.GetClient(cg->clientNum)->name,
				Engine.GetClient(closestPlayer)->name) != 0)
		{
			if (timeChanged == 0)
			{
				strcpy((char*)Offsets::namegame, Engine.GetClient(closestPlayer)->name);
