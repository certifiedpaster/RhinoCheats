...
bool Entity_t::IsValid(int i)
{
	if (cg_entities[i].valid && (cg_entities[i].IsAlive & 1) && (cg->clientNum != i))
	{
		...
		/*{			
			weapon_t *pMyWeapon = Engine.GetCombatWeapon(cg_entities[i].nextState.weaponID);
...
bool Entity_t::IsEnemy(int i)
{
	...
	{
		if (Engine.GetClient(i)->team == Engine.GetClient(cg->clientNum)->team)
			return true;
	...
	if (Engine.GetClient(i)->team != Engine.GetClient(cg->clientNum)->team 
		|| Engine.GetClient(i)->team == TEAM_FREE)
		return true;
...
void Entity_t::DrawEsp()
{
	...
	for (int i = 0; i < cgs->maxClients; ++i)
	{
		if (IsValid(i))
		{
			...
			if (Settings[esp_radar].Value.bValue)
			{
				...
				DrawRotatedPic(GSM(), Radar.screenX, Radar.screenY, 15.0f, 15.0f, Radar.fRadarAngle, vColor, *(int *)Offsets::hud_dpad_arrow /*CompassPlayerIcon*/);
			}
			...
			{				
					...
					if (Settings[esp_box].Value.iValue != 6) //All except with Lilli's boxes.
					{
						if (!Settings[esp_name_boxed].Value.bValue)
						{
							...
							if (colVisible.x == 1.0f
								...
								&& colVisible.z == 1.0f)
							{
								Drawing.DrawGlowString(screen[0], screen[1] - 10, Fonts->hudsmallfont, FONTSCALE, ImVec4(1.0f, 1.0f, 1.0f, 1.0f), ImVec4(0.0f, 0.0f, 0.0f, 0.5f), 0, true, Engine.GetClient(i)->name);
								if (cg_entities[i].nextState.weaponID == 3 || cg_entities[i].nextState.weaponID == 4)
							...
							{
								Drawing.DrawGlowString(screen[0], screen[1] - 10, Fonts->hudsmallfont, FONTSCALE, ImVec4(1.0f, 1.0f, 1.0f, 1.0f), colVisible, 0, true, Engine.GetClient(i)->name);
								if (cg_entities[i].nextState.weaponID == 3 || cg_entities[i].nextState.weaponID == 4)
							...
							Drawing.DrawStringBoxed(screen[0], screen[1] - 13, fonts_Dev->smallDevFont, 1, ImVec4(1.0f, 1.0f, 1.0f, 1.0f), 0, true, colVisible, ImVec4(colVisible.x, colVisible.y, colVisible.z, 0.2f), base[0] - dX / 2, dX, "%s", Engine.GetClient(i)->name);
						}						
					}					
				}
				...
				{		
					...
					case 5:
						vec3_t Min;
						vec3_t Max;
						if (cg_entities[i].currentState.eFlags & 8)
						{
						...
						else if (cg_entities[i].currentState.eFlags & 4)
						{
				...
				if (Settings[esp_weapon].Value.bValue)
					Drawing.DrawShader(base[0] - 24, base[1] + 3, 45, 20, colVisible, Engine.GetCombatWeapon(cg_entities[i].nextState.weaponID)->Icon);
	...
	{
		if (cg_entities[i].nextState.eType != ET_ITEM	    &&
			cg_entities[i].nextState.eType != ET_MISSILE    &&
			cg_entities[i].nextState.eType != ET_TURRET     &&
			cg_entities[i].nextState.eType != ET_HELICOPTER &&
			cg_entities[i].nextState.eType != ET_PLANE      &&
			cg_entities[i].nextState.eType != ET_VEHICLE    &&
			cg_entities[i].nextState.eType != ET_VEHICLE_COLLMAP &&
			cg_entities[i].nextState.eType != ET_VEHICLE_CORPSE  &&
			cg_entities[i].nextState.eType != ET_VEHICLE_SPAWNER	
			)
			continue;
		...
		if (Settings[esp_radar].Value.bValue)
		{			
			switch (cg_entities[i].nextState.eType)
			{
			case ET_ITEM:
		...
		icon = Engine.GetCombatWeapon(cg_entities[i].nextState.weaponID)->Icon;
		...
		case ET_MISSILE:
			...
			if (Engine.WorldToScreen(cg_entities[i].lerpOrigin, base))
			{
				...
				{
					char *pSzName = Engine.GetCombatWeapon(cg_entities[i].nextState.weaponID)->szName;
					if (pSzName != nullptr && strstr(pSzName, "flash"))
					{
		...
		case ET_VEHICLE_SPAWNER:
			...
			if (Engine.WorldToScreen(cg_entities[i].lerpOrigin, base))
			{
				...
				{
					Drawing.DrawGlowString(base[0], base[1], Fonts->hudsmallfont, FONTSCALE, ImVec4(1.0f, 1.0f, 1.0f, 1.0f), misc_color, 0, true, "[Dangerous]");
				}
			}
		}
	}
