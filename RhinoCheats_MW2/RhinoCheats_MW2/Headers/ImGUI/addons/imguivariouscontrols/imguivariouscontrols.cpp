...
inline static bool ColorChooserInternal(ImVec4 *pColorOut,bool supportsAlpha,bool showSliders,ImGuiWindowFlags extra_flags=0,bool* pisAnyItemActive=NULL,float windowWidth = 180/*,bool isCombo = false*/)    {
    ...
    ImVec4 color = *pColorOut;
    if (!supportsAlpha) color.w=1.f;

    ImGuiContext& g = *GImGui;
    float smallWidth = windowWidth/9.f;
    ...
    //const float quadSize = windowWidth - smallWidth - colorWindow->WindowPadding.x*2; Takashi ->
	const float quadSize = windowWidth - smallWidth - colorWindow->WindowPadding.x * 2 - 3;
    ...
    if (ImGui::BeginChild("ValueSaturationQuad##ValueSaturationQuadColorChooser", ImVec2(quadSize, quadSize), false,extra_flags ))   
    {
        ...
        bool hovered, held;
        ImGui::ButtonBehavior(bb, id, &hovered, &held, ImGuiButtonFlags_NoKeyModifiers);
		...
		//Takashi, getting the new color selector :) ->

		ImGuiWindow* window = GetCurrentWindow();
		ImVec2 picker_pos = window->Pos;
		float sv_picker_size = window->Size.x;		
		...
		if (IsItemActive())
		{
			sat = ImSaturate((io.MousePos.x - picker_pos.x) / (sv_picker_size - 1));
			val = 1.0f - ImSaturate((io.MousePos.y - picker_pos.y) / (sv_picker_size - 1));
			colorSelected = true;
		}

		if (colorSelected)
			ColorConvertHSVtoRGB(hue >= 1.0f ? hue - 10 * 1e-6f : hue, sat > 0.0f ? sat : 10 * 1e-6f, val > 0.0f ? val : 1e-6f, color.x, color.y, color.z);

		ImDrawList* draw_list = window->DrawList;
		ImVec4 hue_color_f(1, 1, 1, 1); ColorConvertHSVtoRGB(hue, 1, 1, hue_color_f.x, hue_color_f.y, hue_color_f.z);
		ImU32 hue_color32 = ColorConvertFloat4ToU32(hue_color_f);
		ImU32 col32_no_alpha = ColorConvertFloat4ToU32(ImVec4(color.x, color.y, color.z, 1.0f));
		...
		sv_cursor_pos.x = ImClamp((float)(int)(picker_pos.x + ImSaturate(sat)     * sv_picker_size + 0.5f), picker_pos.x + 2, picker_pos.x + sv_picker_size - 2); // Sneakily prevent the circle to stick out too much
		sv_cursor_pos.y = ImClamp((float)(int)(picker_pos.y + ImSaturate(1 - val) * sv_picker_size + 0.5f), picker_pos.y + 2, picker_pos.y + sv_picker_size - 2);

		// Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
		float sv_cursor_rad = colorSelected ? 10.0f : 6.0f;
    ...
    if (ImGui::BeginChild("Tint##TintColorChooser", ImVec2(smallWidth, quadSize), false,extra_flags))  
    {
        ...
        ImRect bb(window->Pos, window->Pos + window->Size);
        ButtonBehavior(bb, id, &hovered, &held,ImGuiButtonFlags_NoKeyModifiers);
        if (hovered) ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
        if (held)
        {
		...
		ImVec2 picker_pos = window->Pos + ImVec2(0.5f, 0);
		float sv_picker_size = quadSize;
		float bars_width = GetWindowWidth() - 0.5f;//GetFrameHeight();
		float bars_triangles_half_sz = (float)(int)(bars_width * 0.25f);			

		// Hue bar logic
		SetCursorScreenPos(picker_pos);
		InvisibleButton("hue", ImVec2(sv_picker_size, sv_picker_size));
		if (IsItemActive())
		{
			hue = ImSaturate((io.MousePos.y - picker_pos.y) / (sv_picker_size - 1));
			colorSelected = true;
		}
		...
		// Render Hue Bar
		for (int i = 0; i < 6; ++i)
			draw_list->AddRectFilledMultiColor(ImVec2(picker_pos.x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(picker_pos.x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), hue_colors[i], hue_colors[i], hue_colors[i + 1], hue_colors[i + 1]);
		float bar0_line_y = (float)(int)(picker_pos.y + hue * sv_picker_size + 0.5f);
		RenderFrameBorder(ImVec2(picker_pos.x, picker_pos.y), ImVec2(picker_pos.x + bars_width, picker_pos.y + sv_picker_size), 0.0f);
		RenderArrowsForVerticalBar(draw_list, ImVec2(picker_pos.x - 1, bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f);
    ...
    if (showSliders)
    {
        ...
        ImGui::Separator();
        const ImVec2 sliderSize = ImVec2(-1,-1);
...
bool ColorCombo(const char* label,ImVec4 *pColorOut,bool supportsAlpha,float width,bool closeWhenMouseLeavesIt)    {
    ...
    if (ImGui::IsPopupOpen(id))
    {
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 0);
        //ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(4,2)); Takashi ->
