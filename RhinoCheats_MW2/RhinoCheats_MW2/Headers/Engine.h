...
typedef void(__cdecl* BG_GetSpreadForWeapon_t)(cg_t* cg, CWeapon * weapon, float*BaseSpread, float*MaxSpread);
...
typedef char* (__cdecl *ScrPlace_GetActivePlacement_t)(int i);
extern ScrPlace_GetActivePlacement_t GSM;

typedef bool(__cdecl *CG_WorldPosToScreenPosReal_t)(int, char*, vec3_t, float*);
extern CG_WorldPosToScreenPosReal_t WorldToScreenE;

typedef int(__cdecl* CG_DrawRotatedPicPhysical_t) (char* ptr, float x, float y, float w, float h, float angle, float* color, int shader);
...
typedef int(__cdecl *BG_WeaponBulletFire_GetMethodOfDeath_t)(clientInfo_t *a1, CWeapon* a2);
extern BG_WeaponBulletFire_GetMethodOfDeath_t pWeaponImpacttype;

typedef float(__cdecl * BG_GetSurfacePenetrationDepth_t)(CWeapon* a1, int iMaterialType);
...
typedef bool(__cdecl *tsub_52A190)(clientInfo_t *a1, unsigned int a2, int a3);
...
public:		
	...
	// mw2:	
	void      R_BulletPenetration(int unk1, BulletFireParams* bulletTrace, int WeaponNum, centity_t * cent, int unk5, vec3_t vieworg, int unk7);
