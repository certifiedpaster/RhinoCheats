...
namespace ImGui
{
    ...
    // Styles
	IMGUI_API void          StyleColorsNeutral(ImGuiStyle* dst = NULL);
	...
	IMGUI_API void          StyleColorsPurple(ImGuiStyle* dst = NULL);

	IMGUI_API void          StyleCursorsBlack();
	IMGUI_API void          StyleCursorsWhite();
    ...
    // - The disk functions are automatically called if io.IniFilename != NULL (default is NULL).
...
struct ImGuiIO
{
    ...
    const char* IniFilename;                    // = NULL			// Path to .ini file. Set NULL to disable automatic .ini loading/saving, if e.g. you want to manually load/save from memory.
    const char* LogFilename;                    // = NULL			// Path to .log file (default parameter to ImGui::LogToFile when no file is specified).
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont*           AddFontLight(const ImFontConfig* font_cfg = NULL);
	IMGUI_API ImFont*			AddFontMedium(const ImFontConfig* font_cfg = NULL);
	IMGUI_API ImFont*			AddFontBold(const ImFontConfig* font_cfg = NULL);
