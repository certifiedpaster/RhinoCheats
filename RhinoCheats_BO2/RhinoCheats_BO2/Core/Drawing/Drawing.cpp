...
namespace RhinoCheats {
	...
	void cDrawing::DrawBones(
		ImVec2 bones2d[BONE_MAX],
		bool skeleton,
		ImVec4 color
	) {
		if (skeleton) {
	...
	*/
	void cDrawing::DrawBox(
		...
		if (border) {
			ImGui::GetWindowDrawList()->AddRect(ImVec2(x, y), ImVec2(x + w, y + h), ImGui::GetColorU32(color));
		}

		else {
			ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(x, y), ImVec2(x + w, y + h), ImGui::GetColorU32(color));
		}
	}
	/*
	//=====================================================================================
	*/
	void cDrawing::DrawLine(
		float x1,
		float y1,
		float x2,
		float y2,
	...
	*/
	void cDrawing::DrawPlayer(
		...
		else if (_profiler.gPlayerBones.Custom.iValue == cProfiler::PLAYER_BONES_SKELETONS)
			DrawBones(bones2d, true, color);

		if (_profiler.gPlayerLines.Custom.bValue)
		...
		if (_profiler.gPlayerWeapons.Custom.bValue) {
			ImVec2 vStringSize = ImGui::GetIO().FontDefault->CalcTextSizeA(14.0f, FLT_MAX, 0.0f, _targetList.EntityList[entity->iClientNum].szWeapon.c_str());
			DrawString(_targetList.EntityList[entity->iClientNum].szWeapon, center[0] - vStringSize.x / 2.0f, center[1] + flHeight / 2.0f + flPadding, 14.0f, false, color);
		}
	}
	/*
	//=====================================================================================
	*/
	bool cDrawing::CalculateESP(
	...
	void cDrawing::ESP() {
		for (int i = 0; i < MAX_CLIENTS; i++) {
			if (_targetList.EntityList[i].bW2SSuccess && _targetList.EntityList[i].bIsValid) {
				DrawPlayer(&CG->CEntity[i], _targetList.EntityList[i].vBones2D, _targetList.EntityList[i].vCenter, _targetList.EntityList[i].vLower[1] - _targetList.EntityList[i].vUpper[1], GetDistance3D(CG->CEntity[i].vOrigin, CG->vOrigin), CG->Client[i].szName, _targetList.EntityList[i].cColor);
			}
		}
	}
}
