...
#   endif //CACHE_LAST_GRADIENT
    ...
    const int fcgi = (const int)(fillColorGradientDeltaIn0_05*255.0f);
    ...
    if (g<0) g=0;
    if (b<0) b=0;
    if (negative) tc = IM_COL32(0,0,0,A); else bc = IM_COL32(0,0,0,A);
...
inline static ImU32 ColorMergeWithAlpha(ImU32 c,float alphaMult) {
    ...
    ImU32 alpha = (ImU32)(((float)(c>>24))*alphaMult);
...
TabLabelStyle::TabLabelStyle()    {
    ...
    TabLabelStyleSetSelectedTabColors(*this,ImColor(0.267f,0.282f,0.396f,1.0f),ImColor(0.925f,0.945f,0.957f,1.0f),ImColor(0.090f,0.106f,0.157f,0.000f));
...
static bool TabButton(const char *label, bool selected, bool *pCloseButtonPressedOut=NULL, const char* textOverrideIn=NULL, ImVec2 *pJustReturnItsSizeHereOut=NULL, const TabLabelStyle* pOptionalStyleToUseIn=NULL,ImFont *fontOverride=NULL, ImVec2 *pOptionalJustDrawTabButtonGraphicsUnderMouseWithThisOffset=NULL, ImDrawList *drawListOverride=NULL,bool privateReuseLastCalculatedLabelSizeDoNotUse = false,bool forceActiveColorLook = false,ImVec2 *pOptionalTabSize = NULL)  {
    ...
    ImVec2 size = pOptionalTabSize ? *pOptionalTabSize : ImVec2(label_size.x + (style.FramePadding.x+tabStyle.borderWidth) * 2.0f, label_size.y + (style.FramePadding.y+tabStyle.borderWidth) * 2.0f);
            ...
            for (int i = 0; i < numTabs; i++)
            {
                ...
                if (tab.mustCloseNextFrame || mustCloseTab) {
                    ...
                    if (isDraggingCorrectly) {
                        ...
                        const int len1 = (const int)strlen(g.HoveredRootWindow->Name);
                        const int len2 = (const int)strlen(g.CurrentWindow->Name);
...
void TabWindow::render()
{
    ...
    if (g.HoveredRootWindow==ImGui::GetCurrentWindow())
    {
        ...
        if (dd.draggingTabSrc) {
            ...
            {
                const int matchLen = (const int)(match-hoveredWindow->Name+4);
...
bool TabLabels(int numTabs, const char** tabLabels, int& selectedIndex, const char** tabLabelTooltips, bool wrapMode, int *pOptionalHoveredIndex, int* pOptionalItemOrdering, bool allowTabReorder, bool allowTabClosing, int *pOptionalClosedTabIndex, int *pOptionalClosedTabIndexInsideItemOrdering, ImVec2 *pOptionalTabSize) {
    ImGuiStyle& style = ImGui::GetStyle();
    ...
    for (int j = 0,i; j < numTabs; j++)
    {
        ...
        if (!wrapMode) {if (!noButtonDrawn) ImGui::SameLine();canUseSizeOptimization=false;}
            ...
            sumX+=style.ItemSpacing.x;   // Maybe we can skip it if we use SameLine(0,0) below
            ImGui::TabButton(tabLabels[i],(i == selectedIndex),allowTabClosing ? &mustCloseTab : NULL,NULL,&tabButtonSz,&tabStyle,(ImFont *)0,(ImVec2 *)0,(ImDrawList *)0,false,false,pOptionalTabSize);
            sumX+=tabButtonSz.x;
            if (sumX>windowWidth) sumX = 0.f;
            else ImGui::SameLine();
            canUseSizeOptimization = true;
        ...
        if (ImGui::TabButton(tabLabels[i],i == selectedIndex,allowTabClosing ? &mustCloseTab : NULL,NULL,NULL,&tabStyle,NULL,NULL,NULL,canUseSizeOptimization,false,pOptionalTabSize))   {
    ...
    if (draggingTabIndex>=0 && draggingTabIndex<numTabs) {
        ...
        if (ImGui::IsMouseHoveringRect(startGroupCursorPos,startGroupCursorPos+groupSize))  {
            ...
            ImGui::TabButton(tabLabels[pOptionalItemOrdering[draggingTabIndex]],draggingTabWasSelected,allowTabClosing ? &mustCloseTab : NULL,NULL,NULL,&tabStyle,fontOverride,&start,drawList,false,true,pOptionalTabSize);
            ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
...
// Main method to draw the tab label
// The TabLabelStyle used by this method won't be merged with the Window Alpha (please provide a pOptionalStyleToUseIn using TabLabelStyle::GetMergedWithWindowAlpha() if needed).
static bool TabButtonVertical(bool rotateCCW,const char *label, bool selected, bool *pCloseButtonPressedOut=NULL, const char* textOverrideIn=NULL, ImVec2 *pJustReturnItsSizeHereOut=NULL, const TabLabelStyle* pOptionalStyleToUseIn=NULL,ImFont *fontOverride=NULL, ImVec2 *pOptionalJustDrawTabButtonGraphicsUnderMouseWithThisOffset=NULL, ImDrawList *drawListOverride=NULL,bool privateReuseLastCalculatedLabelSizeDoNotUse = false,bool forceActiveColorLook = false,bool invertRounding=false,ImVec2 *pOptionalTabSize=NULL)  {
    // Based on ImGui::ButtonEx(...)
    bool *pHoveredOut = NULL;           // removed from args (can be queried from outside)
    bool *pCloseButtonHovered = NULL;   // removed from args (who cares if the close button is hovered?)
    const int flags = 0;                // what's this ?
...
bool TabLabelsVertical(bool textIsRotatedCCW, int numTabs, const char** tabLabels, int& selectedIndex, const char** tabLabelTooltips, int* pOptionalHoveredIndex, int* pOptionalItemOrdering, bool allowTabReorder, bool allowTabClosing, int* pOptionalClosedTabIndex, int * pOptionalClosedTabIndexInsideItemOrdering, bool invertRounding, ImVec2 * pOptionalTabSize)    {
    ...
    for (int j = 0,i; j < numTabs; j++)
    {
        ...
        // Draw the button
        ImGui::PushID(i);   // otherwise two tabs with the same name would clash.
        if (ImGui::TabButtonVertical(textIsRotatedCCW,tabLabels[i],i == selectedIndex,allowTabClosing ? &mustCloseTab : NULL,NULL,NULL,&tabStyle,NULL,NULL,NULL,canUseSizeOptimization,false,invertRounding,pOptionalTabSize))   {
            selection_changed = (selectedIndex!=i);
            newSelectedIndex = i;
        }
        ImGui::PopID();
    ...
    if (draggingTabIndex>=0 && draggingTabIndex<numTabs) {
        ...
        if (ImGui::IsMouseHoveringRect(startGroupCursorPos,startGroupCursorPos+groupSize))  {
            ...
            const TabLabelStyle& tabStyle = TabLabelStyleGetMergedWithAlphaForOverlayUsage();
            ImFont* fontOverride = (ImFont*) (draggingTabWasSelected ? TabLabelStyle::ImGuiFonts[tabStyle.fontStyles[TabLabelStyle::TAB_STATE_SELECTED]] : TabLabelStyle::ImGuiFonts[tabStyle.fontStyles[TabLabelStyle::TAB_STATE_NORMAL]]);
            ImGui::TabButtonVertical(textIsRotatedCCW,tabLabels[pOptionalItemOrdering[draggingTabIndex]],draggingTabWasSelected,allowTabClosing ? &mustCloseTab : NULL,NULL,NULL,&tabStyle,fontOverride,&start,drawList,false,true,invertRounding,pOptionalTabSize);
            ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
