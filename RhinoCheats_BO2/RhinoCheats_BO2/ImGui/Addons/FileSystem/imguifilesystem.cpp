...
#   endif // _WIN32
    ...
    static void GetDirectoryName(const char *filePath, char *rv) {
        ...
        const int sz = (const int)strlen(filePath);
    ...
    static inline bool HasZipExtension(const char* filePath) {
        ...
        if (!p1) return false;
        const int len = (const int)strlen(p1);
...
#   ifdef _WIN32
        while ((len = strlen(tex))>0 && tex[len-1]=='/') {
            ...
            String::Substr(tex2,tex,0,(int)(len-1));
        }
#   endif //_WIN32
        if (len==0) return;
        int beg=-1;
...
#       endif //_WIN32
        ...
        if (sz>0 && directoryName[sz-1] == '/') String::Substr(directoryName,directoryNameWithoutSlash,0,(int)(sz-1));
	    ...
	    FilenameStringVector wExts;String::Split(wext,wExts,';');
            ...
            if (wExtsSize>0)	{
                for (size_t i = 0,sz = filesIn.size();i<sz;i++)	{
                    Path::GetExtension(filesIn[(int)i],ext);
                    for (size_t e=0;e<wExtsSize;e++)	{
                        if (strcmp(ext,wExts[(int)e])==0) {
                            String::PushBack(files,filesIn[(int)i]);
                            if (pOptionalNamesOut) String::PushBack(*pOptionalNamesOut,namesIn[(int)i]);
                        }
                    }
                }
            }
...
#       ifdef _WIN32
        ...
        for (int i=0,msz=mediaFolders.size();i<msz;i++)    {
            ...
            String::Substr(mediaFolder,mediaFolderName,0,(int)(strlen(mediaFolder)-1));
...
#   endif //IMGUI_USE_MINIZIP
    ...
    if (openInTextMode) {bufferOut.resize((int)(sz+1));bufferOut[(int)sz]='\0';}
    else bufferOut.resize((int)sz);
    if (sz>0)   {        
...
struct ImGuiFsDrawIconStruct {
    ...
    int getExtensionType(const char* ext,bool caseSensitiveMatch=false) const {
        if (!ext) return -1;
        if (ext[0]=='.') ext+=1;
        const int extLen = (const int)strlen(ext);
...
#                   endif //(defined(__EMSCRIPTEN__) && defined(EMSCRIPTEN_SAVE_SHELL))
    ...
    if (isSaveFileDialog || isSelectFolderDialog)   {
        ...
        if (isSaveFileDialog)   {
            ...
            static const ImVec4 sf(1.0f,0.8f,0.5f,1.f);      // selected folder color factor
